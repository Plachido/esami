...
def login():
    ...
    if username in users and users[username] == password:
        ...
        session['answers'] = [None] * len(questions)  # Initialize answers list here
...
def exam():
    ...
    # Load the selected answer for the current question
    selected_answer = session['answers'][question_index]

    return render_template('exam.html', question=question, options=shuffled_options, index=question_index, total=len(session['questions']), selected_answer=selected_answer)
...
def submit_answer():
    ...
    session_answers[question_index] = selected_answer
    session['answers'] = session_answers  # Re-assign to trigger session update
